...
公式ドキュメントによると、 **2026年7月12日現在はS3、SQS、DynamoDB、Lambda、API Gateway、CloudFormation、KMS、CloudFrontなどを含む69サービスを掲載しています** 。
...
このスタックで最も重要なのは、アプリケーションコードではなくCDK側で環境差を吸収した点です。
...
### 詰まった点：同名のstateful resourceが衝突する
...
実AWSとFlociでデータ層を分けた部分が、最も大きな設計判断です。
...
複雑な構成になるほど、作成より削除順序の方が難しくなります。マルチリージョン版ではRouter→4アプリ→データ層の順序も固定しました。
...
2. **flociとCDKを併用する場合は環境毎の差異やデプロイ先の切り替えをスプリクトにラップする**
