---
title: "さよなら、CAPTCHA。AIエージェントが自律的に支払う「MPP（Machine Payments Protocol）」の衝撃"
emoji: "🤖"
type: "tech" # tech: 技術記事 / idea: アイデア
topics: ["MPP", "Stripe", "Tempo", "Web3", "Blockchain"]
published: false
---
...
- **アグノスティックな設計**: Tempo（ステーブルコイン）、Stripe（カード）、Lightning（BTC）など、決済レールを選ばない。
...
1. **Challenge**: サーバーは「いくら、どの通貨で、どの決済レールで支払ってほしいか」を返します。
2. **Credential**: クライアントはそれに基づき支払いを実行し、「支払い証明」を付けて再送します。
3. **Receipt**: サーバーは検証し、リソースと共に「受領証」を返します。
...
- **MPP**: x402の思想を継承しつつ、**Stripe（カード決済）やLightning Networkなども包含した、より実用的で汎用的な「オープン標準」**としてStripe/Tempoが共同策定したものです。
...
まずは、公式の `mppx` CLIを使って、AIエージェントにお財布を持たせてみることから始めてみませんか？

決済の歴史は、今まさに「人間」から「マシン」へと、その主役を広げようとしています。
