---
title: "AIエージェントに「両替」を。Sera Protocol × Tempo MPP で実現する究極のクロスボーダー決済"
emoji: "💱"
type: "tech"
...
これまで連載形式で **Sera Protocol** の技術を追いかけ、MCPサーバーを組み、[Agent SKILL](https://zenn.dev/mashharuki/articles/web3_sera_protocol-7) まで作り込んできました。その熱意が届いたのか、この度正式に **Sera Protocol のエバンジェリスト** に就任いたしました！🚀

私が Sera に惚れ込んだ理由は、単に「板取引ができる DEX だから」ではありません。**「AIエージェントが自律的に経済活動を行うための、最後のピース」** だと確信したからです。

今日は、その確信を証明する話をします。ターゲットは、Stripe と Tempo が提唱する **Machine Payments Protocol (MPP)** との融合です。

https://zenn.dev/mashharuki/articles/mpp-machine-payments-protocol

MPP によって AI は「支払う手」を手に入れました。しかし、国境を越えた瞬間に **「通貨の壁」** という冷たい現実が立ちはだかります。この壁を Sera Protocol がどう粉砕するのか。エバンジェリストとしての初陣、全力で語らせてください。
...
- **APIサーバー（米国）**: 「この画像生成は **$0.01 USDX**（USDステーブルコイン）だよ」
- **AIエージェント（日本）**: 「自分、**JPYC**（日本円ステーブルコイン）しか持ってないっす...」

人間なら「クレカが裏でやってくれるでしょ」で済みますが、自律的に動く AI エージェントにとって、この通貨の不一致は **「死（サービス利用不可）」** を意味します。

マシンの世界では、誰かがオンチェーンで、かつ瞬時に両替を代行しなければなりません。これが、マシンのための外貨両替 **「OnChain FX」** の必然性です。
