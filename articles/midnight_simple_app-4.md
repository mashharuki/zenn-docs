...
「グー」を選んでトランザクションを投げた時点で、相手のウォレットはもう私の手をブロックチェーンの台帳から読み取れてしまう。これではゲームにならない。コミット・リビール方式を自前で実装しようとしたが、ソルトの管理、フロントランニング対策、判定ロジックの公平性……考えれば考えるほど沼だった。

そこで出会ったのが **Midnight** だ。Cardano のプライバシー保護サイドチェーンで、「秘密は手元に、証明だけをチェーンに送る」という設計が言語レベルで組み込まれている。じゃんけんの「後出し禁止」を数学的に保証できる。

本記事では、Midnight のスマートコントラクト言語 **Compact** で実装した Rock-Paper-Scissors（RPS）dApp を題材に、ZK 回路の設計思想と実際に開発してぶつかった壁を共有する。

> **⚠️ ZK が守る範囲について**: ZK 証明が保証するのは「コミット段階での手の秘匿性」＝フェアネスです。reveal 後、双方の手はパブリック台帳に記録されます。完全な匿名性とは異なる点をご注意ください。
...
手元で動かすまでの全手順をまとめた。正直に言うと、**ZK 回路のコンパイルと Docker イメージの初回 Pull を含めると合計 15〜20 分**はかかる。覚悟して進んでほしい。
...
> `bun contract compact` は Compact コンパイラが ZK 証明回路（proving key / verification key）を生成するステップです。マシンスペックによっては 10 分近くかかることがあります。

### 3. Proof Server の起動（初回 Docker Pull：〜3分）

ZK 証明の生成を担うサーバーを Docker で立ち上げる。このサーバーなしでは commit / reveal のトランザクションが送れない。

> バージョン `8.0.3` は Compact `0.2.0` との動作確認済みバージョン。Midnight の SDK とバージョンを揃えないと証明生成に失敗するので注意。
...
![Architecture Components](/images/midnight_simple_app-4/1.png)
...
秘密鍵から公開鍵を導出する際、Midnight では以下のパターンが推奨される。
...
私が実装中に最初に迻った設計判断のひとつだ。そのままま使っていたら、別のアプリで同じ秘密鍵を使用したときに公開鍵が同一になり、プライバシー漏洩のリスクが生まれる。「アプリ名 + バージョン」の固定プレフィックスを混ぜることで、同じ秘密鍵から生成される公開鍵がアプリごとに必ず異なる。
...
ここで導入した **`store_move_and_salt`** に注目してほしい。これは witness 関数の一つで、選んだ手とソルトをブラウザの IndexedDB（プライベートステート）に保存する処理だ。reveal 時に同じ手とソルトで「コミットした内容と一致する」ことを ZK 証明するため、この 1 行なしではゲームが成立しない。

`disclose()` を使っている点も重要で、ZK 回路内の「秘密」の計算結果のうち、どれを「公開」するかを開発者が厳密にコントロールできる。

#### ゲームの状態遷移

commit / reveal の 2 フェーズがどう連鎖するかを状態遷移で整理する。
...
コミットが「宣言」なら、リビールは「証明」だ。ここが Midnight の ZK がフルに活きる部分。
...
`assert(disclose(computed == p1_commit), "Commitment mismatch for P1")` この 1 行こそが ZK の核心だ。
...
reveal が完了して初めて `p1_move` / `p2_move` がオンチェーンに書き込まれ、`who_wins()` で勝敗が確定する。コミット段階では相手の手は永遠に分からない。これが「後出し禁止」の仕組みだ。
...
![Game Result](/images/midnight_simple_app-4s/4.png)

Midnight と Compact の設計は、実際に榽を作って初めて分かることが多い。今回実装して残った課題を正直に書いておく。

**1. ゲーム放棄問題（Griefing Attack）** 

P1 がコミットしたまま P2 が永遠に参加しない場合、コントラクトがロックされる。タイムアウト・キャンセルの仕組みが今の実装にはなく、次のアップデートで対処したい。

**2. reveal 後のプライバシーの誤解**

記事中にも追記したが、`p1_move` / `p2_move` は reveal 後にパブリック台帳に書き込まれる。「匿名じゃんけん」と言いたが、ZK が保証するのはコミット段階のフェアネスであり完全匿名性ではない。読者に誤解を与えた表現だった。

**3. ビルド資産のバージョン管理**

`bun contract compact` で生成される ZK 回路資産（proving key）は、Compact のバージョンやコンパイラの微妙な変化で無効になる。フロントエンドにバンドルする資産とデプロイ済みコントラクトの整合性を局面ごとに検証するフローがまだ整っていない。

この 3 つの課題を解決すれば、**Midnight**を活用した実用レベルの ZK dApp に近づきそう！

---

## まとめ

実装を通じて、4 つの設計原則が身についた。

1. **情報の境界線を引く**: 常に「何が秘密で、何が公開か」を問い続ける。
2. **`store_move_and_salt` を流さない**: reveal のためのプライベートステート保存は不可欠。
3. **UX を最優先する**: ZK 特有の待ち時間を、ユーザーとの対話に変える。
4. **ZK が守る範囲を正確に伝える**: 「匿名」ではなく「フェアネスの保証」だ。
